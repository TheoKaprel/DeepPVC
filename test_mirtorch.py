...
def main():
    ...
    asum = A_noRM._apply_adjoint(torch.ones_like(projs_tensor_mir))
    asum[asum == 0] = float('Inf')
    out = torch.ones_like(asum)

    for iter in range(args.niter):
        print(f'iter : {iter}')
        ybar = A_noRM._apply(out)
        ybar[ybar==0] = 1
        yratio = torch.div(projs_tensor_mir, ybar)
        back = A_noRM._apply_adjoint(yratio)
        out = torch.multiply(out, torch.div(back, asum))
    ...
    rec_corrected_fp = A_RM._apply(out)

    itk.imwrite(itk.image_from_array(out.detach().cpu().numpy()), os.path.join(args.output, 'rec.mhd'))
    itk.imwrite(itk.image_from_array(rec_corrected_fp.detach().cpu().numpy()), os.path.join(args.output, 'rec_fp.mhd'))
