...
def eval_error(lpth, input,dataset_path,type,ref, verbose):
    ...
    for pth in lpth:
        ...
        if input:
            ...
            test_dataset = dataset.CustomPVEProjectionsDataset(params=params, paths=[dataset_path], test=True)
        else:
...
def eval_plot(lpth, input, n, dataset_path, type,merged, ref, verbose, param_comp):
    ...
    for (pth_id,pth) in enumerate(lpth):
        ...
        if input:
            ...
            test_dataset = dataset.CustomPVEProjectionsDataset(params=params, paths=[dataset_path],filetype=type,merged=merged)
        else:
            print('ERROR : no input nor dataset specified. You need to specify EITHER a --input /path/to/input OR a number -n 10 of image to select randomly in the dataset')
            exit(0)
